...
void AItemSpawner::BeginPlay()
{
	...
	if (HasAuthority())
	{
		...
		GetWorldTimerManager().SetTimer(ItemChangeTimerHandle, FTimerDelegate::CreateUObject(this, &ThisClass::ChangeActiveItemIndex), ItemChangeDelay, true, 0.f);
	}
}
...
void AItemSpawner::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	{
		// 첫번째 아이템을 미리보기로 표시한다.
		// 아이템이 설정되지 않았으면 표시하지 않는다.
		const int32 FirstItemIndex = ItemInfos.IsEmpty() ? -1 : 0; 
		ShowItem(FirstItemIndex);
	}
}
#endif

void AItemSpawner::ChangeActiveItemIndex()
{
...
void AItemSpawner::ShowItem(int32 ItemIndex)
{
	if (ItemIndex >= 0)
	{
		ItemMeshComponent->SetStaticMesh(ItemInfos[ItemIndex].ItemMesh);
		ItemMeshComponent->SetRelativeTransform(ItemInfos[ItemIndex].ItemMeshTransform);
		SphereComponent->SetSphereRadius(ItemInfos[ItemIndex].SphereRadius);
		SphereComponent->SetRelativeLocation(FVector(0.f, 0.f, ItemInfos[ItemIndex].SphereRadius));
	}
	else
	{
